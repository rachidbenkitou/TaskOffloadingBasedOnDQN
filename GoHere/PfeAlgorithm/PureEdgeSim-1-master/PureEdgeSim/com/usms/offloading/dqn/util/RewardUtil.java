...
public class RewardUtil {
    ...
    public double getLatencyLocal(long containerSize, double dij) {
        return ((s.getLength() / s.getLocalComputationRate()) + ((containerSize / dataRate(BwDeviceEdge, dij)) + (containerSize / dataRate(BwEdgeCloud, dij))) * (1 - a.getActionCode()[0])); // Changed index to 1
    }

    public double getLatencyEdge(long containerSize, double dij) {
        return ((s.getRequestSize() / dataRate(BwDeviceEdge, dij)) + ((containerSize / dataRate(BwEdgeCloud, dij)) * (1 - a.getActionCode()[1])) + (s.getLength() / s.getEdgeComputationRate())); // Changed index to 1
    }
    ...
    public double getEnergyLocal(long containerSize, double dij) {
        return ((switchedCapacitance * s.getLength() * Math.pow(s.getLocalComputationRate(), 2)) +
                ((1 - a.getActionCode()[0]) * ((powerTransmission * containerSize / dataRate(BwDeviceEdge, dij)) + (powerTransmission * containerSize / dataRate(BwEdgeCloud, dij))))
        );
    }
    ...
    public double getEnergyEdge(long containerSize, double dij) {
        ...
        return ((s.getRequestSize() * powerTransmission / dataRate(BwDeviceEdge, dij))
                + ((powerTransmission * containerSize / dataRate(BwEdgeCloud, dij)) * (1 - a.getActionCode()[1])) +
