...
class DeepQLearningAgent:
    def __init__(self, state_size, action_size, learning_rate=0.001, gamma=0.95, update_target_network_period=10,
        ...
        self.memory = deque(maxlen=self.replayBufferSize)
    ...
    def train(self):
        """Trains the network for a fixed number of time steps."""
        for experience in self.memory:
            state, action, reward, next_state = experience

            # Compute the target Q-value
            target = self.main_network.predict(np.array([state]))
            next_Q_values = self.target_network.predict(np.array([next_state]))[0]
            target[0][action] = reward + self.gamma * np.max(next_Q_values)

            # Train the main network on the computed target
